...
- **Tier-0 (user-paste) is not a substitute for tier-2** — a side-by-side comparison in spike 003b revealed that user-pasted DOM text MISSED 6 of 8 videos, all 3 critic reviews, and several structured cross-refs (because users paste what they highlight, not the entire page). Tier 0 demoted to "last-resort escape hatch" — production should always prefer tier 2 when extension is available. Added 2026-05-08 from spike 003b.
- **Tier-2 (Claude_in_Chrome) returns the entire DOM in one call** — `get_page_text` is the gold-standard ingest API when available. ~10s end-to-end including page load. No image-embedded text (use tier 3 for that). Added 2026-05-08 from spike 003b.
- **Production primary path is tier 2 + optional tier 3 follow-up for image-rich pages** — tier 2 gets structured DOM; tier 3 captures visual content (product-photo control labels, screen-state demos) when those matter. Both paths skip Cloudflare. Added 2026-05-08 from spike 003b.
...
| 003b | tier2-claude-in-chrome-escalation | comparison | Real browser via Claude_in_Chrome MCP can ingest a Cloudflare-blocked URL when extension is connected — and returns full DOM in one call | ✓ VALIDATED | research, web-scrape, tier-2, claude-in-chrome |
