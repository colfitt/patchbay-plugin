...
- [x] **RESEARCH-04**: User can review `failures.log` via a sub-command (`/patchbay:research --review-failures`) and choose, per entry, to escalate to tier 2, tier 3, paste manually, or skip. **No auto-fallback** between tiers.
- [x] **RESEARCH-05**: Tier-2 escalation prechecks `mcp__Claude_in_Chrome__list_connected_browsers`; if empty, surfaces extension install instructions instead of failing silently.
