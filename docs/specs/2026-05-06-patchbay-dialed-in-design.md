# patchbay:dialed-in — Design Spec
...
Save and generate gear dial-in sessions as structured, searchable files. When a user approximates a target tone using owned gear, `dialed-in` produces a markdown file with knob positions, toggle states, signal chain context, and technique notes — all anchored to a specific song and gear substitution.
...
**Follow-up from `liner-notes`:** if invoked in the same session as a `liner-notes` run, song context (artist, song, Applied section) is already loaded — skip SongProfile lookup.
...
Identify artist, song title. Load `SongProfile.md` from `<songs_root>/<Artist>/<song-slug>/`. If no SongProfile exists, stop:
...
**If user specified gear:** go directly to Step 4 for that substitution.
...
For each selected substitution, check if `dial-in/<owned-slug>--<target-slug>.md` exists.

If it does, present re-run menu:
...
**Signal chain context** — if the role involves an amp or interface, include settings for owned amp/interface (e.g. Iridium) alongside the pedal settings.
